...
class PerfilViewModel : ViewModel() {
        ...
        repository.cargarDatosUsuario(uid,
            onSuccess = { doc ->
                ...
                (doc.get("descripcionesPorCategoria") as? Map<*, *>)?.entries?.forEach { entry ->
                    ...
                    if (clave != null && valor != null) descripcionesMap[clave] = valor
                }
                _uiState.value = PerfilUiState.SuccessCargarPerfil(doc)
            },
            onFailure = { e ->
                _uiState.value = PerfilUiState.Error("Error al cargar perfil: ${e.message}")
            }
        )
    }

    fun cargarResenas(usuarioId: String, categoriaFiltro: String? = null) {
        if (!categoriaFiltro.isNullOrEmpty()) {
            repository.cargarResenasPorCategoria(usuarioId, categoriaFiltro,
                onSuccess = { lista ->
                    _uiState.value = PerfilUiState.SuccessCargarResenas(lista)
                },
                onFailure = { e ->
                    _uiState.value = PerfilUiState.Error("Error al cargar reseñas: ${e.message}")
                }
            )
        } else {
    ...
    fun guardarCambiosPerfil(nombre: String, telefono: String, fotoPerfilB64: String?) {
        ...
        repository.actualizarCamposPerfil(uid, updates,
            onSuccess = {
                _uiState.value = PerfilUiState.SuccessOperacion("Perfil actualizado con éxito")
            },
            onFailure = { e ->
    ...
    fun eliminarQRBilletera(esYape: Boolean) {
        ...
        repository.actualizarCamposPerfil(
            uid,
            mapOf(campoAEditar to ""),
    ...
    fun subirFotoPortafolio(b64: String, categoria: String, uid: String) {
        val fotoId    = "${categoria}_${System.currentTimeMillis()}"
    ...
    fun recuperarPortafolioDinamico(
        uid: String,
        esModoLectura: Boolean,
        categoriaFiltro: String?,
        onResult: (List<FotoTrabajo>) -> Unit
    ) {
        if (esModoLectura && !categoriaFiltro.isNullOrEmpty()) {
            ...
            repository.escucharPortafolioCompleto(uid) { snap ->
